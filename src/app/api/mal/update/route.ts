...
// POST { animeId, status?, episodes?, score? } — writes back to the MAL list.
export async function POST(req: NextRequest) {
  ...
  try {
    const { tokens } = await callWithAuth(access, refresh, (t) =>
      updateAnimeStatus(t, animeId, {
        status: body?.status,
        num_watched_episodes: body?.episodes != null ? Number(body.episodes) : undefined,
